...
namespace VRTK
{
    ...
    public class VRTK_SnapRotateObjectControlAction : VRTK_BaseObjectControlAction
    {
        ...
        protected override void Process(GameObject controlledGameObject, Transform directionDevice, Vector3 axisDirection, float axis, float deadzone, bool currentlyFalling, bool modifierActive)
        {
            ...
            if (!turned && snapDelayTimer < Time.time && ValidThreshold(axis))
            {
            ...
            // modifying this clunky code to work properly :(
            // Debug.Log(axis); thats some good shit!
            if(Mathf.Abs(axis) < axisThreshold + 0.1f)
            {
                turned = false;
            }
